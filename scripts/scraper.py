...
def scrape_sz_finance():
    ...
    news = []
    for item in soup.select('.list li, .news-list li, .list-item, .article-list li')[:10]:
        a_tag = item.select_one('a')
        if not a_tag: continue
        title = a_tag.get_text(strip=True)
        href = a_tag.get('href', '')
        if not title or not href: continue
        if not href.startswith('http'): href = 'https://jr.sz.gov.cn' + href
        date_span = item.select_one('.date, .time')
        date = date_span.get_text(strip=True) if date_span else ''
        if '金融' in title or '银行' in title or '外资' in title:
...
def scrape_sz_stats():
    ...
    for item in soup.select('.list li, .news-list li, .list-item, .article-list li')[:10]:
        ...
        href = a_tag.get('href', '')
        if not title or not href: continue
        if not href.startswith('http'): href = 'https://tjj.sz.gov.cn' + href
        date_span = item.select_one('.date, .time')
        date = date_span.get_text(strip=True) if date_span else ''
        if '金融' in title or '银行' in title or '存款' in title or '贷款' in title:
...
def scrape_safe():
    ...
    soup = BeautifulSoup(resp.text, 'lxml')
    news = []
    for item in soup.select('.list li, .list-item, .main .list a')[:10]:
        a_tag = item.select_one('a')
        if not a_tag: continue
        title = a_tag.get_text(strip=True)
        href = a_tag.get('href', '')
        ...
        if not href.startswith('http'):
            if href.startswith('./'): href = 'https://www.safe.gov.cn/shenzhen/' + href[2:]
            elif href.startswith('/'): href = 'https://www.safe.gov.cn' + href
            else: href = 'https://www.safe.gov.cn/shenzhen/' + href
        date_span = item.select_one('.date, .time')
        date = date_span.get_text(strip=True) if date_span else ''
...
def scrape_pbc():
    ...
    soup = BeautifulSoup(resp.text, 'lxml')
    news = []
    for item in soup.select('.list li, .news-list li, .list-item')[:10]:
        a_tag = item.select_one('a')
        if not a_tag: continue
        title = a_tag.get_text(strip=True)
        href = a_tag.get('href', '')
        if not title or not href: continue
        if not href.startswith('http'):
            if href.startswith('../../'): href = 'https://shenzhen.pbc.gov.cn/shenzhen/122807/' + href.replace('../../', '').replace('./', '')
            elif href.startswith('/'): href = 'https://shenzhen.pbc.gov.cn' + href
            else: href = 'https://shenzhen.pbc.gov.cn/shenzhen/122807/' + href
        date_span = item.select_one('.date, .time')
        date = date_span.get_text(strip=True) if date_span else ''
...
def scrape_nfra():
    ...
    soup = BeautifulSoup(resp.text, 'lxml')
    news = []
    for item in soup.select('.list li, .news-list li, .list-item, .index-list li')[:10]:
        a_tag = item.select_one('a')
        if not a_tag: continue
        title = a_tag.get_text(strip=True)
        href = a_tag.get('href', '')
        if not title or not href: continue
        if not href.startswith('http'): href = 'https://www.nfra.gov.cn' + href
        date_span = item.select_one('.date, .time')
        date = date_span.get_text(strip=True) if date_span else ''
...
def main():
    ...
    all_news = {'gov_policies': [], 'reg_policies': [], 'last_update': datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
